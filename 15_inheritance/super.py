class Vehicle:
    def __init__(self, brand, year):
        self.brand = brand
        self.year = year
        print(f"차량 ({self.brand}, {self.year}년식)이 생성 되었습니다.")
...
class Car(Vehicle):
    def __init__(self, brand, year, color):
        ...
        print(f"자동차 ({self.brand}, {self.year}년식, {self.color})가 생성 되었습니다.")
...
# 오토바이 클래스 Motorcycle (Vehicle 상속 받음)
...
class Motorcycle(Vehicle):
    def __init__(self, brand, year, type):
        super().__init__(brand, year)
        self.type = type
        print(f"오토바이 ({self.brand}, {self.year}년식, {self.type})가 생성 되었습니다.")
...
# class Manager
# department
# display_info => 부모 꺼 호출해서 출력 하고 department 따로 또 출력
