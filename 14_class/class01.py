# pass는 파이썬에서 아무 작업도 수행 하지 않는 빈 코드 블록을 만들 때 사용
# 문법적으로는 특정 블록이 필요하지만, 실제로 수행할 작업이 없으 때 허용 됨.
# class Car:
#     pass

class Car:
    def __init__(self, model, price): # 생성자, 객체가 생성될 때 자동으로 실행
        # self는 현재 생성 되는 객체 자신을 의미
        self.model = model
        self.price = price
    ...
    def drive(self, speed, distance): # speed, distance는 외부에서 전달 받는 매개변수
        # self는 이 메소드를 호출 하는 객체 자기 자신을 참조
